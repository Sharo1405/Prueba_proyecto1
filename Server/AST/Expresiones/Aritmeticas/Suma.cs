...
{
    class Suma : Operacion, Expresion
    {
        ...
        public object getValue(Entorno entorno, ErrorImpresion listas)
        {
            ...
            {
                return Convert.ToString(expresion1.getValue(entorno, listas)) + Convert.ToString(expresion2.getValue(entorno, listas));
            }
            else if (nuevo == tipoDato.entero)
            {
            ...
            else
            {
                listas.errores.AddLast(new NodoError(this.linea, this.columna, NodoError.tipoError.Semantico, "Tipo de dato para operador \"+\" no valido Tipos: " +
                    Convert.ToString(expresion1.getType(entorno, listas)) + " y " + Convert.ToString(expresion2.getType(entorno, listas)) + " y se esperaba Int o Double o String"));
